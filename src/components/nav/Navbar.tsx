...
export function Navbar() {
  ...
  // off the same attribute, so nothing here has to run for the menu to work.
  useEffect(() => {
    const root = document.documentElement;
    const sync = () => setOpen(root.getAttribute("data-menu-open") === "true");
    sync();
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span className="relative block h-[30px] w-[117px] sm:h-9 sm:w-[140px]">
            ...
            <motion.span
              ...
              transition={{ duration: 0.5, ease: easing.gentle }}
            >
              <Logo variant="mono" height={36} className="h-full w-auto" />
            </motion.span>
          </span>
        </a>
        ...
        {/* No onClick: a delegated listener installed in <head> owns this,
            so the first tap works long before React hydrates. Both icons are
            rendered and swapped in CSS for the same reason. */}
        <button
          id="menu-toggle"
          type="button"
        ...
        >
          <Icon name="menu" className="icon-open h-5 w-5" />
          <Icon name="close" className="icon-close h-5 w-5" />
        </button>
      </div>

      {/* Always in the DOM and hidden in CSS — mounting it on state would put
          it back behind hydration, which is the whole problem being solved. */}
      <div
        id="mobile-menu"
      ...
      >
        ...
        <nav className="relative flex h-full flex-col justify-center gap-1 overflow-y-auto px-6 pb-10 pt-24">
          {navLinks.map((link) => {
            const isActive = active === link.href.slice(1);
            return (
              <a
                key={link.href}
                href={link.href}
                className="menu-item flex items-center justify-between border-b border-hair py-5 font-serif text-3xl font-light text-ink transition-colors active:text-acc"
                style={isActive ? { color: "var(--acc)" } : undefined}
              >
                {link.label}
          ...
          <div className="menu-item mt-10">
            <Button href="#contact" variant="fill" size="md" className="w-full">
              Get a Quote
            </Button>
          </div>
        </nav>
